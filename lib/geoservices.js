...
var addParamsToPath = function(options) {
  ...
  options.path += '?' + url.format(query);
};

var serializeObject = function(obj) {
  var result = {};
  _.map(obj, function(val, key) {
    result[key] = RequestParams.serializeValue(val);
  });

  return _.reduce(result, function(memo, val, key) {
    return memo += '&' + key + '=' + val;
  }, '').substring(1);
};
...
module.exports = {
  ...
  post: function(options, callback) {
    ...
    var body = serializeObject(options.params);
    var req = http.request(options);
    req.on('response', function(res) {
      var result = '';
      res.on('data', function(data) {
